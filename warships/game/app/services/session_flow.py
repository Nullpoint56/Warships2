...
from engine.api.flow import FlowTransition, create_flow_machine
...
class SessionFlowService:
    ...
    def resolve(current_state: AppState, trigger: str) -> AppTransition | None:
        """Resolve navigation trigger using engine flow primitives."""
        machine = create_flow_machine(current_state)
        ...
        if not machine.trigger(trigger):
            return None
