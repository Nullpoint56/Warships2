...
def prompt_buttons(prompt: PromptView) -> list[Button]:
    ...
    return project_buttons(
        (
            ButtonSpec(prompt.confirm_button_id, confirm.x, confirm.y, confirm.w, confirm.h),
            ButtonSpec(prompt.cancel_button_id, cancel.x, cancel.y, cancel.w, cancel.h),
        )
    )
...
def preset_row_buttons(rows: list[PresetRowView]) -> list[Button]:
    ...
    for idx, row in enumerate(rows):
        ...
        specs.append(
            ButtonSpec(
                f"preset_edit:{row.name}", edit_rect.x, edit_rect.y, edit_rect.w, edit_rect.h
            )
        )
        specs.append(
            ButtonSpec(
                f"preset_rename:{row.name}",
                rename_rect.x,
                rename_rect.y,
                rename_rect.w,
                rename_rect.h,
...
def new_game_setup_buttons(
    *,
    ...
    specs.append(
        ButtonSpec(
            "new_game_toggle_difficulty", difficulty.x, difficulty.y, difficulty.w, difficulty.h
        )
    )
    if difficulty_open:
        for idx, name in enumerate(DIFFICULTIES):
            rect = NEW_GAME_SETUP.difficulty_option_rect(idx)
            specs.append(ButtonSpec(f"new_game_diff_option:{name}", rect.x, rect.y, rect.w, rect.h))

    for idx, name in enumerate(
        PresetFlowService.visible_new_game_preset_names(rows, scroll, visible_rows)
    ):
        row_rect = NEW_GAME_SETUP.preset_row_rect(idx)
        specs.append(
            ButtonSpec(
                f"new_game_select_preset:{name}", row_rect.x, row_rect.y, row_rect.w, row_rect.h
            )
        )
