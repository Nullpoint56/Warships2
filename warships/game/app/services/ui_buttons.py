...
def preset_row_buttons(rows: list[PresetRowView]) -> list[Button]:
    ...
    for idx, row in enumerate(rows):
        ...
        buttons.append(
            Button(f"preset_edit:{row.name}", edit_rect.x, edit_rect.y, edit_rect.w, edit_rect.h)
        )
        buttons.append(
            Button(
...
def new_game_setup_buttons(
    *,
    ...
    buttons.append(
        Button("new_game_toggle_difficulty", difficulty.x, difficulty.y, difficulty.w, difficulty.h)
    )
    if difficulty_open:
    ...
    for idx, name in enumerate(
        ...
        buttons.append(
            Button(f"new_game_select_preset:{name}", row_rect.x, row_rect.y, row_rect.w, row_rect.h)
        )
    random_rect = NEW_GAME_SETUP.random_button_rect()
    buttons.append(
        Button("new_game_randomize", random_rect.x, random_rect.y, random_rect.w, random_rect.h)
    )
    return buttons
