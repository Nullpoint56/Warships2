...
def test_target_aliases_and_unknown_target() -> None:
    layout = GridLayout()
    assert layout.rect_for_target("player").x == layout.primary_origin_x
    assert layout.rect_for_target("enemy").x == layout.secondary_origin_x
...
def test_screen_to_cell_bounds_and_edges() -> None:
    ...
    assert layout.screen_to_cell("primary", 0, 0) == layout.screen_to_cell("self", 0, 0)
