...
def test_buttons_for_state_presence() -> None:
    ...
    assert any(b.id == "start_game" for b in buttons_for_state(AppState.NEW_GAME_SETUP, False, False))
    assert any(b.id == "create_preset" for b in buttons_for_state(AppState.PRESET_MANAGE, False, True))
